...
const EnrollmentList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.get('/payment-methods').then(r => setPaymentMethods((r.data.data || []).filter((m: PaymentMethod) => m.is_enabled))).catch(() => {});
                ...
                <select className="form-control" value={payForm.payment_method_id} onChange={e => setPayForm(p => ({ ...p, payment_method_id: e.target.value }))}>
                  ...
                  {paymentMethods.map(method => (
                    <option key={method.id} value={method.id}>
                      ...
                      {method.upi_id ? ` — ${method.upi_id}` : method.bank_name ? ` — ${method.bank_name}` : ''}
                    </option>
                  ))}
                </select>
              </div>
